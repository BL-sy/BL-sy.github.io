...
- 事件处理集成：实现ImGuiLayer::OnEvent，将引擎的键盘 / 鼠标事件传递给 ImGui（如鼠标点击 UI 时，游戏逻辑不响应）；
- 自定义样式：修改 ImGui 的颜色、字体、控件大小，使其与引擎风格一致；
- 编辑器功能扩展：基于 ImGui 开发场景编辑器（物体选择、属性修改）、性能监控面板（帧率、内存占用）；
- 多上下文支持：为不同模块（引擎调试、游戏 UI）创建独立 ImGui 上下文，避免状态冲突。
